...
printf '%s\0' mpy/*/ | xargs -0 -L1 bash -c 'cd -- "$1" && bash ./build.sh && cp *.so ../../userconf/plugins/.' _;
printf 'Building lang plugins\n';
printf '%s\0' mpy/lang/*/ | xargs -0 -L1 bash -c 'cd -- "$1" && bash ./build.sh && cp *.so ../../../userconf/plugins/lang/.' _;
printf 'Building style plugins\n'
printf '%s\0' mpy/styles/*/ | xargs -0 -L1 bash -c 'cd -- "$1" && bash ./build.sh && cp *.so ../../../userconf/plugins/styles/.' _;
printf 'Building syntax plugins\n';
printf '%s\0' mpy/lang/syntax/*/ | xargs -0 -L1 bash -c 'cd -- "$1" && bash ./build.sh && cp *.so ../../../../userconf/plugins/lang/syntax/.' _;
printf 'Building lang tools plugins\n';
printf '%s\0' mpy/lang/tools/*/ | xargs -0 -L1 bash -c 'cd -- "$1" && bash ./build.sh && cp *.so ../../../../userconf/plugins/lang/tools/.' _;
